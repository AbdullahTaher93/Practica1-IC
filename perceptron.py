...
input={}
weight = [ [0 for columna in range(785)] for fila in range (10)]#28*28 +1 (bias)
output={}
LEARNING_RATE = 0.055
BIAS=255#normalizado queda en 1

for i in range(785):#28*28 +1 (bias)
    input[i]=0

for i in range(10):
    output[i]=0
    for j in range(785):#28*28 +1 (bias)
...
for n in range(len(t_train)):
    ...
    input_rgb=np.append(BIAS,x_train[n]) #a la imagen agrega un elemento adicional correspondiente a bias
    #print("input_rgb",input_rgb)
    #print('largo input_rgb ',len(input_rgb))
    inputImg=function_rn.normalizaImg(input_rgb)
    #print('largo inputImg ',len(inputImg))
    #son 10 neuronas
    for i in range(10):
...
for n in range(len(t_test)):
    ...
    numbrerv=function_rn.number2vector(target_numero)
    input_rgb=np.append(BIAS,x_test[n]) #a la imagen agrega un elemento adicional correspondiente a bias
    inputImg=function_rn.normalizaImg(input_rgb)
    #son 10 neuronas
    for i in range(10):
        weight_neuro=weight[i]
